...
(
    ...
    printf '{"container":"%s","instanceId":"%s","ip":"%s","state":"FAILED","reason":"PORT_NOT_DETECTED"}\n' \
        "$CONTAINER_NAME" "$IID" "$PRIVATE_IP" > /tmp/executor-port-timeout.json

    aws --region "$REGION" s3 cp /tmp/executor-port-timeout.json \
